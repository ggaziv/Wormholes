...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
                random_start=False, random_restarts=False, do_tqdm=False, miniters=None,
        ...
        def get_adv_examples(x):
            ...
            if do_tqdm:
                iterator = tqdm(iterator, miniters=miniters)
...
def interp(x, target, alpha, constraint, eps):
    ...
    # Initialize step class and attacker criterion
    step_class = STEPS[constraint] if isinstance(constraint, str) else constraint
    step = step_class(eps=eps, orig_input=orig_input, step_size=None)

    x = x.clone().detach()
    with ch.no_grad():
        x = interp_images(x, target, alpha=alpha)
        x = step.project(x)
    return step.to_image(x.clone().detach())
...
class AttackerModel(ch.nn.Module):
    ...
    def predict(self, im, is_latent=False, return_numpy=True):
        res = self.forward(im.cuda(), with_latent=is_latent)[0]
        if is_latent:
            res = res[1]
        res = res.cpu().detach()
        return res.numpy() if return_numpy else res

    def attack(self, im, label, **kwargs):
        return self.forward(im.cuda(), label.cuda(), make_adv=True, **kwargs)[-1].cpu()

...
import time
import warnings
from wormholes.perturb import get_norm
...
def eval_model(args, model, loader, store):
    ...
    prec1, nat_loss, nat_budget_usage = _model_loop(args, 'val', loader, 
    ...
    if args.adv_eval: 
        ...
        adv_prec1, adv_loss, adv_budget_usage = _model_loop(args, 'val', loader, 
    ...
    log_info = {
        ...
        'nat_loss':nat_loss,
        'adv_loss':adv_loss,
        'nat_budget_usage':nat_budget_usage,
        'adv_budget_usage':adv_budget_usage,
        'train_prec1':float('nan'),
        'train_loss':float('nan'),
        'train_budget_usage':float('nan'),
...
def train_model(args, model, loaders, *, checkpoint=None, dp_device_ids=None,
    ...
    for epoch in range(start_epoch, args.epochs):
        # train for one epoch
        train_prec1, train_loss, train_budget_usage = _model_loop(args, 'train', train_loader, 
        ...
        if should_log or last_epoch or should_save_ckpt:
            ...
            with ctx:
                prec1, nat_loss, nat_budget_usage = _model_loop(args, 'val', val_loader, model, 
            ...
            adv_prec1, adv_loss, adv_budget_usage = adv_val or (-1.0, -1.0, -1.0)
            ...
            log_info = {
                ...
                'train_budget_usage':train_budget_usage,
                'time': time.time() - start_time
            }
...
def _model_loop(args, loop_type, loader, model, opt, epoch, adv, writer):
    ...
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    iterator = tqdm(enumerate(loader), total=len(loader), miniters=int(223265/100))
    for i, (inp, target) in iterator:
       # measure data loading time
        ...
        if adv:
            budget_usage_batch = get_norm(final_inp, inp, p=float(args.constraint)) / eps
        ...
        try:
            ...
            if adv:
                budget_usage.update(budget_usage_batch.mean(), inp.size(0))
            
            top1_acc = top1.avg
            top5_acc = top5.avg
            budget_usage_avg = budget_usage.avg
        except:
    ...
    if writer is not None:
        ...
        descs = ['loss', 'top1', 'top5', 'budget_usage']
        vals = [losses, top1, top5, budget_usage]
        for d, v in zip(descs, vals):
            writer.add_scalar('_'.join([prec_type, loop_type, d]), v.avg,
                              epoch)

    return top1.avg, losses.avg, budget_usage.avg

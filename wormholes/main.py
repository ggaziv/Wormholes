...
try:
    from .model_utils import (make_and_restore_model, 
                              get_restricted_imagenet_mapped_model)
...
parser = defaults.add_args_to_parser([['map_imagenet_model_to_restricted_imagenet', [0, 1], 'map an imagenet model to restricted imagenet', 0]], parser)
...
def main(args, store=None):
    ...
    # MAKE MODEL
    if args.map_imagenet_model_to_restricted_imagenet:
        model, checkpoint = get_restricted_imagenet_mapped_model(args.arch, dataset, args.pytorch_pretrained, args.resume)
    else:
        model, checkpoint = make_and_restore_model(arch=args.arch, dataset=dataset, 
                                                   pytorch_pretrained=args.pytorch_pretrained, resume_path=args.resume)
...
def setup_args(args):
    ...
    if args.eval_only: assert args.resume is not None or args.pytorch_pretrained, \
            "Must provide a resume path or pretrained model if only evaluating"
    return args
...
def setup_store_with_metadata(args):
    ...
    if not args.eval_only and not args.resume:  # Condition needed to avoid bug https://github.com/MadryLab/cox/issues/4
